...
export default function ConsultorTicketDetalhePage() {
  ...
  async function carregar() {
    try {
      ...
      const res = await fetch(`/api/consultor/tickets/${ticketId}`, {
        cache: "no-store",
      });
      ...
      setAnexos(Array.isArray(json?.anexos) ? json.anexos : []);
    } catch (e: any) {
      setErro(e?.message || "Erro ao carregar ticket.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function alterarEstado(estado: string) {
    try {
      ...
      const res = await fetch(`/api/consultor/tickets/${ticketId}/estado`, {
        method: "POST",
        headers: {
      ...
      alert(e?.message || "Erro ao alterar estado.");
    } finally {
  ...
  async function enviarMensagem() {
    try {
      ...
      const res = await fetch(`/api/consultor/tickets/${ticketId}/mensagens`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  if (loading) {
    return (
      <main style={styles.page}>
        <div style={styles.card}>A carregar ticket...</div>
      </main>
    );
  }
  ...
  if (erro || !ticket) {
    return (
      <main style={styles.page}>
        <div style={styles.err}>{erro || "Ticket não encontrado."}</div>
      </main>
    );
  }
  ...
  return (
    <main style={styles.page}>
      ...
      <div style={styles.grid}>
        ...
        <section style={styles.card}>
          <h2 style={styles.h2}>Mensagens</h2>

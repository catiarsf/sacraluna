...
export default function ClienteTicketsPage() {
  ...
  async function carregar() {
    try {
      ...
      if (!res.ok || !json?.ok) {
        throw new Error(json?.error || "Erro ao carregar os teus serviços.");
      }

      setTickets(Array.isArray(json?.tickets) ? json.tickets : []);
    } catch (e: any) {
      setErro(e?.message || "Erro ao carregar os teus serviços.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main style={styles.page}>
      <div style={styles.topRow}>
        <div>
          <h1 style={styles.h1}>Os meus serviços</h1>
          <p style={styles.sub}>Aqui acompanhas os serviços que compraste.</p>
        </div>

        <div style={styles.actions}>
          <Link href="/cliente" style={styles.backBtn}>
            ← Voltar
          </Link>
      ...
      {loading ? (
        <div style={styles.card}>A carregar serviços...</div>
      ) : tickets.length === 0 ? (
        <div style={styles.card}>Ainda não tens serviços comprados.</div>
      ) : (
              ...
              <div style={styles.cardTop}>
                <div>
                  ...
                  <div style={styles.meta}>
                    <b>Consultora:</b> {ticket.consultor_nome || "-"}
                  </div>

                  <div style={styles.meta}>
                    <b>Comprado em:</b> {formatDate(ticket.created_at)}
                  </div>
                  ...
                  {ticket.entregue_at ? (
                    <div style={styles.meta}>
                      <b>Entregue em:</b> {formatDate(ticket.entregue_at)}
                    </div>
                  ) : null}
                </div>
                ...
                <div style={styles.sideBox}>
                  ...
                  <Link href={`/cliente/tickets/${ticket.id}`} style={styles.openBtn}>
                    Abrir pedido
                  </Link>
                </div>
              </div>
